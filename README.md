...
- **Strict Priority & Delayed Delivery**  
  Messages are dequeued by priority (lower number = higher) and `deliver_after` timestamp.
...
- **Dynamic Topics**  
  No pre-creation needed — topics are auto-discovered.
...
+-------------------+       +---------------------+       +-------------------+
|   Producers       |       |     mqueue API      |       |    Consumers      |
| (n8n, apps, etc.) | <---> |  (HTTPS + TLS)      | <---> |  (Workers)        |
+-------------------+       |  /enqueue           |       +-------------------+
                            |  /dequeue           |
                            |  /ack /nack /dlq    |
                            +----------+----------+
